<div class="settingSidebar">
    <a href="javascript:void(0)" class="settingPanelToggle">
        <i class="fa fa-spin fa-cog"></i>
    </a>
    <div class="settingSidebar-body ps-container ps-theme-default">
        <div class="fade show active">
            <div class="setting-panel-header">Панель настроек</div>

            <div class="p-15 border-bottom">
                <h6 class="font-medium m-b-10">Выбор макета</h6>
                <div class="selectgroup layout-color w-50">
                    <label class="selectgroup-item">
                        <input type="radio" name="value" value="1"
                            class="selectgroup-input-radio select-layout" checked>
                        <span class="selectgroup-button">Светлый</span>
                    </label>
                    <label class="selectgroup-item">
                        <input type="radio" name="value" value="2"
                            class="selectgroup-input-radio select-layout">
                        <span class="selectgroup-button">Тёмный</span>
                    </label>
                </div>
            </div>

            <div class="p-15 border-bottom">
                <h6 class="font-medium m-b-10">Цвет боковой панели</h6>
                <div class="selectgroup selectgroup-pills sidebar-color">
                    <label class="selectgroup-item">
                        <input type="radio" name="icon-input" value="1"
                            class="selectgroup-input select-sidebar">
                        <span class="selectgroup-button selectgroup-button-icon"
                            data-toggle="tooltip" data-original-title="Светлая боковая панель">
                            <i class="fas fa-sun"></i>
                        </span>
                    </label>
                    <label class="selectgroup-item">
                        <input type="radio" name="icon-input" value="2"
                            class="selectgroup-input select-sidebar" checked>
                        <span class="selectgroup-button selectgroup-button-icon"
                            data-toggle="tooltip" data-original-title="Тёмная боковая панель">
                            <i class="fas fa-moon"></i>
                        </span>
                    </label>
                </div>
            </div>

            <div class="p-15 border-bottom">
                <h6 class="font-medium m-b-10">Цветовая тема</h6>
                <div class="theme-setting-options">
                    <ul class="choose-theme list-unstyled mb-0">
                        <li title="белый" class="active">
                            <div class="white"></div>
                        </li>
                        <li title="голубой">
                            <div class="cyan"></div>
                        </li>
                        <li title="чёрный">
                            <div class="black"></div>
                        </li>
                        <li title="фиолетовый">
                            <div class="purple"></div>
                        </li>
                        <li title="оранжевый">
                            <div class="orange"></div>
                        </li>
                        <li title="зелёный">
                            <div class="green"></div>
                        </li>
                        <li title="красный">
                            <div class="red"></div>
                        </li>
                    </ul>
                </div>
            </div>

            <div class="p-15 border-bottom">
                <div class="theme-setting-options">
                    <label class="m-b-0">
                        <input type="checkbox" name="custom-switch-checkbox"
                            class="custom-switch-input" id="mini_sidebar_setting">
                        <span class="custom-switch-indicator"></span>
                        <span class="control-label p-l-10">Мини-боковая панель</span>
                    </label>
                </div>
            </div>

            <div class="p-15 border-bottom">
                <div class="theme-setting-options">
                    <label class="m-b-0">
                        <input type="checkbox" name="custom-switch-checkbox"
                            class="custom-switch-input" id="sticky_header_setting">
                        <span class="custom-switch-indicator"></span>
                        <span class="control-label p-l-10">Закреплённый заголовок</span>
                    </label>
                </div>
            </div>

            <div class="mt-4 mb-4 p-3 align-center rt-sidebar-last-ele">
                <a href="#" class="btn btn-icon icon-left btn-primary btn-restore-theme">
                    <i class="fas fa-undo"></i> Восстановить по умолчанию
                </a>
            </div>
        </div>
    </div>
</div>